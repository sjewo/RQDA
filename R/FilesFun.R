...
ViewFileFunHelper <- function(FileName,hightlight=TRUE,codingTable=.rqda$codingTable, annotation=TRUE){
  ...
  tmp$SetPixelsBelowLines(5) ## set the spacing
  tmp$SetPixelsInsideWrap(5) ## so the text looks more confortable.
  ...
  add(W, content)
  slot(W, "widget")$SetEditable(FALSE)
...
EditFileFun <- function(FileNameWidget=.rqda$.fnames_rqda){
  ...
  if (is_projOpen(envir=.rqda, conName = "qdacon")) {
    ...
    if (length(svalue(FileNameWidget)) == 0) {
      ...
      add(W, content)
      buffer <- slot(W, "widget")$GetBuffer() ## get text buffer.
      ...
      gSignalConnect(.rqda$.openfile_gui$GetBuffer(), "changed",
                     function(h,...){
...
viewPlainFile <- function(FileNameWidget=.rqda$.fnames_rqda){
  ...
  Encoding(content) <- "UTF-8"
  add(tmp, content)
  slot(tmp, "widget")$SetEditable(FALSE)
}}}
